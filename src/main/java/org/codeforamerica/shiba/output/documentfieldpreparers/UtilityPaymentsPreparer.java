...
public class UtilityPaymentsPreparer extends OneToManyDocumentFieldPreparer {
	...
	private static final List<String> UTILITY_PAYMENTS_OPTIONS = List.of("ELECTRICITY", "PHONE", "GARBAGE_REMOVAL");
	...
	protected List<DocumentField> map(PagesData pagesData) {
		...
		// No expedited utilities selected - need multiple inputs because they are
		// written differently
		// to xml and pdfs
		if (utilityPayments.stream().noneMatch(EXPEDITED_UTILITY_PAYMENTS::contains)) {
			results.add(new DocumentField("utilityPayments", "noExpeditedUtilitiesSelected", "true",
		...
		boolean selectedHeating = utilityPayments.contains("HEATING");
		boolean selectedCooling = utilityPayments.contains("COOLING");
