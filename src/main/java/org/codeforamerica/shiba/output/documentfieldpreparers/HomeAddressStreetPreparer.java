...
public class HomeAddressStreetPreparer implements DocumentFieldPreparer {
	...
	public List<DocumentField> prepareDocumentFields(Application application, Document document, Recipient recipient) {
		...
		if (parseBoolean(getFirstValue(pagesData, NO_PERMANENT_ADDRESS))) {
			return createAddressInputs(NO_PERMANENT_ADDRESS_STREET, "", "", "", "MN", "");
		}
		...
		if (document == Document.CCAP && !(parseBoolean(getFirstValue(pagesData, USE_ENRICHED_HOME_ADDRESS)))) {
			...
			return createAddressInputsForCCAP(getFirstValue(pagesData, HOME_STREET),
					getFirstValue(pagesData, HOME_APARTMENT_NUMBER), getFirstValue(pagesData, HOME_ZIPCODE),
					getFirstValue(pagesData, HOME_CITY), getFirstValue(pagesData, HOME_STATE),
		...
		if (parseBoolean(getFirstValue(pagesData, USE_ENRICHED_HOME_ADDRESS))) {
			...
			return createAddressInputs(getFirstValue(pagesData, HOME_STREET),
					getFirstValue(pagesData, HOME_APARTMENT_NUMBER), getFirstValue(pagesData, HOME_ZIPCODE),
					getFirstValue(pagesData, HOME_CITY), getFirstValue(pagesData, HOME_STATE),
					getFirstValue(pagesData, HOME_COUNTY));
		}
	...
	// CCAP-specific version
	private List<DocumentField> createAddressInputsForCCAP(String street, String apartment, String zipcode, String city, String state, String county) {
	    if (County.Other.toString().equals(county)) {
			county = "";
		}
		String streetWithApt = street;
	    ...
	    return List.of(
	        createSingleHomeAddressInput("streetAddressWithPermanentAddress", streetWithApt),
	...
	// Default version
	private List<DocumentField> createAddressInputs(String street, String apartment, String zipcode, String city,
			String state, String county) {
		// county Fields default to "Other" but we don't want to write that to the PDF
		if (County.Other.toString().equals(county)) {
			county = "";
		}
		return List.of(createSingleHomeAddressInput("streetAddressWithPermanentAddress", street),
				createSingleHomeAddressInput("selectedApartmentNumber", apartment),
				createSingleHomeAddressInput("selectedZipCode", zipcode),
				createSingleHomeAddressInput("selectedCity", city),
				createSingleHomeAddressInput("selectedState", state),
