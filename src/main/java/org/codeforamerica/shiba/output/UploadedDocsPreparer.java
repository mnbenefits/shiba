...
public class UploadedDocsPreparer {
   ...
   */
	 ...
	 * Application application) { List<ApplicationFile> applicationFiles = new
	 * ArrayList<>(); byte[] coverPage =
	 * pdfGenerator.generateCoverPageForUploadedDocs(application); for (int i = 0; i
	 * < uploadedDocs.size(); i++) { UploadedDocument originalDocument =
	 * uploadedDocs.get(i); try { ApplicationFile preparedDocument =
	 * pdfGenerator.generateForUploadedDocument(originalDocument, i, application,
	 * coverPage); if (preparedDocument != null &&
	 * preparedDocument.getFileBytes().length > 0) {
	 * applicationFiles.add(preparedDocument); } } catch (NullPointerException e) {
	 ...
	 * pdfGenerator.generateForUploadedDocument(originalDocument, i, application,
	 * null); log.info("Now queueing file to send: %s".formatted(preparedDocument.
	 * getFileName()) + " without cover page");
	 * applicationFiles.add(preparedDocument); } } return applicationFiles; }
	 */
